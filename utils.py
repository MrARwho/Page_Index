...
import httpx
_custom_timeout = httpx.Timeout(3600.0, connect=60.0)
custom_sync_client = httpx.Client(timeout=_custom_timeout)
custom_async_client = httpx.AsyncClient(timeout=_custom_timeout)
...
def llm_completion(model, prompt, chat_history=None, return_finish_reason=False):
    ...
    for i in range(max_retries):
        try:
            print(f"[DEBUG-llm_completion] Attempt {i+1}. Model: {model}. Prompt length: {len(prompt)} chars. Awaiting response...")
            import time
            start_t = time.time()
...
async def llm_acompletion(model, prompt):
    ...
    for i in range(max_retries):
        try:
            print(f"[DEBUG-llm_acompletion] Attempt {i+1}. Model: {model}. Prompt length: {len(prompt)} chars. Awaiting async response...")
            import time
            start_t = time.time()
